...
public class HokuyoProxy extends AmberProxy {
    ...
    private Map<Integer, Scan> scanMap = new ConcurrentHashMap<Integer, Scan>();
    ...
    public void handleDataMsg(CommonProto.DriverHdr header, CommonProto.DriverMsg message) {
        ...
        if (!message.hasAckNum() || message.getAckNum() == 0) {
            ...
            if (scanMap.containsKey(ackNum)) {
                Scan scan = scanMap.remove(ackNum);
    ...
    public void fillScan(Scan scan, CommonProto.DriverMsg message) {
        HokuyoProto.Scan originalScan = message.getExtension(HokuyoProto.scan);

        if (range == null) {
            range = new AngleRange(originalScan.getAnglesList(), 0, 180, 90);
        }

        scan.fillScan(originalScan.getDistancesList(), range);
    }
}
